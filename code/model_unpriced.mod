...
var 
    ...
    rw rmc rph rpf rpm i q pi pih pif y_star piftilde 
    thet rmc_star cs ;
varexo 
    eps_thet ;
parameters 
    ...
    RHO_thet sig_thet 
    coefm coefc 
    CHoY MHoY EoY CFoY_STAR MFoY_STAR KAPPA ;
...
% demand of foreign intermediates 1
...
% import pricing 12
pif = (KAPPA-1)/PHI * (rmc_star + q - rpf) + BETA*pif(+1); 
...
% shadow allocation import input
mf_sh = -ETA * (rpf - rpm) + m ;

% shadow allocation import consumption
cf_sh = -EPS * rpf + c ;
